...
redcap_data_adnin = read.csv("/Users/rabiul/fwdcodeusedinanalysis/RedCapUp2.csv") %>%
  ...
  filter(sa_allc_ckdepi_renal1_v23y10 !=9)%>%   ##############
  na.omit()
...
redcap_data3<-within(redcap_data_adnin,{
  ...
  event <- as.numeric(sa_allc_ckdepi_renal1_v23y10 %in% c(1,2)) #############
