...
#Requirements
requirements <- c("tidyverse","adapr","survival","survminer","dplyr","CBPS","purrr","gtsummary","gt","cobalt","officer","rvg","lme4","lmerTest","effects","emmeans")
#CRAN repository
...
```{r download}
#get data
model_data = load('surv_model_data.Rdata')
```
...
#Optimal cutoff
#T1pats<-as.matrix(rawdata$Adenine..nM.uM.>0.024630967)
#T2pats<-as.matrix(rawdata$Adenine..nM.uM. <=0.024630967)
...
for (year in 1:6){
  ...
  rawdata[paste("outcome40",year, sep = "_")] =((t1-t0)/t0 < -.40) | ((t2==1) & year>= rawdata$sa_allc_cric_time_renal2_v3y0)# ((t1-t0)/t0 < -.40) | (t5==1)
}
```
...
outcome<-c()
#rawdata$outcome<-rawdata$outcome40_1
#Loop over patients
for (i in 1:dim(rawdata)[1]){
  #commulative outcomes from year 1 to 6
  outevent<-rawdata[outcomes[1:5]][i,]
