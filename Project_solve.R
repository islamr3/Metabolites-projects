...
sojib <- function(dt,x){
  ...
  name <- dt %>% 
    ...
    select(patientid,ahh$x, sa_allc_time_esrd_v3y0, sex_v3y0,age_integer_v3y0, 
           albuminuria_v3y0, hemoglobin_a1c_v3y0, map_v3y0,bmi_v3y0,egfr_cric_v3y0,log_acr,adnine) %>%
...
logity1 <- glm(event ~ age_integer_v3y0+hemoglobin_a1c_v3y0+map_v3y0+bmi_v3y0+egfr_cric_v3y0+log_acr+adnine, data = redcap_data3, family = binomial)
