...
#Create quantile
q=quantile(data$adnine, c(1/3, 2/3))
q=as.matrix(q)
#Lower Q1
q1<-subset(data,data$adnine<q[1])

#Middle Q2
q2<-subset(data,data$adnine>=q[1] & data$adnine<=q[2])

#Upper Q1
q3<-subset(data,data$adnine>q[2])
...
ggplot(Lq, aes(x = event, fill = event))+geom_bar()
length(which(Lq$event==1))
length(which(Lq$event==0))
...
#Check data with lebel PV=0.096
ggplot(UPq, aes(x = event, fill = event))+geom_bar()
length(which(UPq$event==1))
length(which(UPq$event==0))



#fit logistic regression model
LqModel <- glm(event ~ age+ha1c+map+bmi+egfr+log_acr+adnine, family="binomial", data=Lq)
UpModel <- glm(event ~ age+ha1c+map+bmi+egfr+log_acr+adnine, family="binomial", data=UPq)
...
#fit Cox's model
Lqcox <- coxph(surv ~ age + ha1c + map + bmi + age + egfr+adnine, data = Lq)
Upcox <- coxph(surv ~ age + ha1c + map + bmi + age + egfr+adnine, data = Lq)
...
EGFR<-redcap_data_adnin%>%
  filter(sex_v3y0==1)%>%
  filter(albuminuria_v3y0==1)%>%
  dplyr::select(egfr_cric_v3y0,egfr_cric_v5y1,egfr_cric_v7y2,egfr_cric_v3y0,egfr_cric_v11y4,egfr_cric_v13y5,egfr_cric_v15y6,egfr_cric_v17y7,
