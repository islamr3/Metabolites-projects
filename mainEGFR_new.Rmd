...
stratanames = c("Normoalbuminuria", "Microalbuminuria")
...
adjustmentnames <- c("no_adj", "min_adjusted", "adjusted", "fully_adjusted") 

```

```{r metrics}
model_df <- data.frame(matrix(nrow=1, ncol=7))
names(model_df) = c("Strata", "Adjustment", "Outcome", "AUC", "95%CI", "Adenine_pvalue", "Likelihood_ratio_test")
...
#loop over strata
for (stidx in 1:length(stratanames)){
  stratum <- strata[[stidx]]
  #loop over adjustment
    for (adjidx in 1:length(adjustmentnames)){

      vars<- adjustments[[adjidx]]
      ...
      for (outcome in outcomes){
        ...
        #calculate reduced model if there is adjustments
        #if(adjidx>1){
          #redmod_formula <- as.formula( paste(outcome, paste(vars[2:length(vars)], collapse = "+"), sep = "~"))
          #redmod <- glm(formula = redmod_formula, family = binomial, data = rawdata[stratum, ])
        #}
      
        #Book keeping columns
        model_df[mod_idx, "Strata"] <- stratanames[stidx]
        model_df[mod_idx, "Adjustment"] <- adjustmentnames[adjidx]
        model_df[mod_idx, "Outcome"] <- outcome

        #return AUC
        model_df[mod_idx, "AUC"] <-lroc(mod, graph=FALSE)$auc
        
        #return Adenine CI
        model_df[mod_idx, "95%CI"] <- paste("[",paste(confint(mod)[2,], collapse=", "),"]") 
        
        #return Adenine p_value
        model_df[mod_idx, "Adenine_pvalue"] <- summary(mod)$coefficients[2,4]
  
        #caluclate likelihood ratio test #to remove predictor  vars[2:length(vars)]
        #if(adjidx>1){
          #model_df[mod_idx, "Likelihood_ratio_test"] <- lrtest(mod, redmod)
        #}
